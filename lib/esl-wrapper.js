...
const MEDIA_ESL_AUTHENTICATION_ERROR = 'authenticationError';
  ...
  _connect () {
    ...
    this._client.auth((error) => {
      if (error) {
        ...
        this.error = this._normalizeError(new Error(MEDIA_ESL_AUTHENTICATION_ERROR), error.message);
      }
    });
  }
    ...
    this._client.on('error', (error) => {
      if (error) {
        Logger.error(`FSESL connection received error ${error.code}`,
          { error });
        this.error = this._normalizeError(new Error(MEDIA_ESL_CONNECTION_ERROR), error.message);
    ...
    return new Promise((resolve, reject) => {
      ...
      this._client.api(command, (res) => {
        ...
        if (this._hasError(body) && !body.includes('no reply')) {
          return reject(this._normalizeError(new Error(MEDIA_ESL_COMMAND_ERROR), body));
        }
        return resolve();
      })
    })
    .catch((e) => {
      Logger.warn(`FSESL command failed: ${command}`, { error: e?.message || e, details: e?.details });
